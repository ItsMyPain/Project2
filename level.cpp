...
bool TileMap::load(const std::string &tmx_file_path) {
    ...
    while (!isalpha(path.front())) // ≈сли путь редактор записал, например, так : "../textures/Tiles.png"
        path.erase(0,
                   1);          // то убираем все лишние символы в начале строки ( ../ ), чтобы избежать сбо€ загрузки
    ...
    for (auto layer = root_element->FirstChildElement("layer");
        ...
        for (size_t y = 0u, index = 0u; y < height; ++y) {
            for (size_t x = 0u; x < width; ++x, ++index) {
                ...
                if (tile_num) {   // ќбходим по часовой стрелке
                    ...
                    rightTop.texCoords = sf::Vector2f(texture_grid[tile_num - 1].x + tile_width,
                                                      texture_grid[tile_num - 1].y);
                    rightBottom.texCoords = sf::Vector2f(texture_grid[tile_num - 1].x + tile_width,
                                                         texture_grid[tile_num - 1].y + tile_height);
                    leftBottom.texCoords = sf::Vector2f(texture_grid[tile_num - 1].x,
                                                        texture_grid[tile_num - 1].y + tile_height);
    ...
    for (auto object_group = root_element->FirstChildElement("objectgroup");
         ...
         object_group = object_group->NextSiblingElement(
                 "objectgroup")) {// ѕолучаем все данные - тип, им€, позици€, etc
